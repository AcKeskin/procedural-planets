...
public:
    ...
    static MeshData GeneratePlanetMesh(int subdivisions, float baseRadius, const std::vector<float>& heights);

    // Generate planet mesh with pre-computed heights and shading data (GPU)
    static MeshData GeneratePlanetMesh(int subdivisions,
                                       float baseRadius,
...
private:
    ...
    static uint32_t GetMidpoint(MeshData& mesh, std::map<uint64_t, uint32_t>& cache, uint32_t i1, uint32_t i2);
